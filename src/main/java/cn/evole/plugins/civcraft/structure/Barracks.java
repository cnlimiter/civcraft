...
import cn.evole.plugins.civcraft.interactive.InteractiveRepairItem;
import cn.evole.plugins.civcraft.items.components.RepairCost;
import cn.evole.plugins.civcraft.loreenhancements.LoreEnhancement;
import cn.evole.plugins.civcraft.loreenhancements.LoreEnhancementNoRepair;
...
import cn.evole.plugins.civcraft.threading.tasks.UnitSaveAsyncTask;
...
public class Barracks extends Structure {
    ...
    private String getUnitSignText(int index) throws IndexOutOfBoundsException {
        ...
        if (unit.id.equals("u_settler")) {
            coinCost = addPriceOfPreviousSettlers(coinCost);
        }
    ...
    private void train(Resident whoClicked) throws CivException {
        ...
        this.updateTraining();
        if (unit.id.equals("u_settler")) {
            CivGlobal.getSessionDB().add("settlers:" + this.getCiv().getId(), "1", this.getTown().getId(),
                    this.getCiv().getId(), this.getId());
        }
        this.onTechUpdate();
    }
    ...
    public void createUnit(ConfigUnit unit) {
        ...
        try {
            String className = "cn.evole.plugins.civcraft.items.units." + unit.class_name;
