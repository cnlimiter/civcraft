...
import cn.evole.plugins.civcraft.config.ConfigIngredient;
import cn.evole.plugins.civcraft.config.ConfigMaterial;
import cn.evole.plugins.civcraft.items.components.ItemComponent;
...
//	public LinkedList<ItemStack> shaplessIngredientList = new LinkedList<ItemStack>();
    ...
    public static String getShapedRecipeKey(ItemStack[] matrix) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            ...
            if (isCustom(stack)) {
                LoreMaterial loreMat = getMaterial(stack);
                key += loreMat.getId() + ",";
            } else {
            ...
            if (loreMat.isShaped()) {
                ...
                for (ConfigIngredient ingred : configMaterial.ingredients.values()) {
                    ...
                    if (ingred.custom_id == null) {
                        ...
                        ingredStack = spawn(customLoreMat);
                    }
...
//		}
    ...
    public static String serializeEnhancements(ItemStack stack) {
        ...
        for (LoreEnhancement enh : getEnhancements(stack)) {
    ...
    private void buildComponents() {
        ...
        if (compInfoList != null) {
            for (HashMap<String, String> compInfo : compInfoList) {
                String className = "cn.evole.plugins.civcraft.items.components." + compInfo.get("name");
                Class<?> someClass;
