...
//        world.loadChunk(loc.getChunk());
        ...
        BukkitObjects.getScheduler().callSyncMethod(CivCraft.getPlugin(), () -> world.addEntity(ent, CreatureSpawnEvent.SpawnReason.CUSTOM));
    ...
    public static void spawnRandomCustomMob(Location loc) {
        ...
        for (ConfigMobs cmob : CivSettings.customMobs.values()) {
            for (String s : cmob.biomes) {
                if (s.equalsIgnoreCase(loc.getBlock().getBiome().toString())) {
                    validMobs.add(cmob);
                }
            }
        }
