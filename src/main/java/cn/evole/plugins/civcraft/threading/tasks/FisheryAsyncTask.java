...
import cn.evole.plugins.civcraft.structure.FishHatchery;
            ...
            for (ItemStack stack : contents0) {
                ...
                if (ItemManager.getId(stack) == CivData.FISHING_ROD) {
                    try {
                        ...
                        this.updateInventory(UpdateInventoryRequest.Action.REMOVE, source_inv0, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                        damage++;
                        if (damage < 64) {
                            this.updateInventory(UpdateInventoryRequest.Action.ADD, source_inv0, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                        }
                    } catch (InterruptedException e) {
                        return;
                    }
            ...
            if (this.fishHatchery.getLevel() >= 2) {
                for (ItemStack stack : contents1) {
                    ...
                    if (ItemManager.getId(stack) == CivData.FISHING_ROD) {
                        try {
                            short damage = ItemManager.getData(stack);
                            this.updateInventory(UpdateInventoryRequest.Action.REMOVE, source_inv1, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                            damage++;
                            if (damage < 64) {
                                this.updateInventory(UpdateInventoryRequest.Action.ADD, source_inv1, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                            }
                        } catch (InterruptedException e) {
                            return;
                        }
            ...
            if (this.fishHatchery.getLevel() >= 3) {
                for (ItemStack stack : contents2) {
                    ...
                    if (ItemManager.getId(stack) == CivData.FISHING_ROD) {
                        try {
                            short damage = ItemManager.getData(stack);
                            this.updateInventory(UpdateInventoryRequest.Action.REMOVE, source_inv2, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                            damage++;
                            if (damage < 64) {
                                this.updateInventory(UpdateInventoryRequest.Action.ADD, source_inv2, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                            }
                        } catch (InterruptedException e) {
                            return;
                        }
            ...
            if (this.fishHatchery.getLevel() >= 4) {
                for (ItemStack stack : contents3) {
                    ...
                    if (ItemManager.getId(stack) == CivData.FISHING_ROD) {
                        try {
                            short damage = ItemManager.getData(stack);
                            this.updateInventory(UpdateInventoryRequest.Action.REMOVE, source_inv3, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                            damage++;
                            if (damage < 64) {
                                this.updateInventory(UpdateInventoryRequest.Action.ADD, source_inv3, ItemManager.createItemStack(CivData.FISHING_ROD, 1, damage));
                            }
                        } catch (InterruptedException e) {
                            return;
                        }
