...
public class DynmapUpdateTask implements Runnable {
    ...
    public DynmapUpdateTask(DynmapAPI api, MarkerAPI markerapi, MarkerSet townset, MarkerSet cultureset, MarkerSet structureSet) {
        this.api = api;
        ...
        while (nodevals != null) {
            ...
            for (TownChunk node : nodevals) {
                ...
                if (ourblks == null) {   /* If not started, switch to world for this block first */
                    if (!node.getChunkCoord().getWorldname().equals(currentWorldName)) {
    ...
    private void updateStructures() {
        ...
        while (iter.hasNext()) {
            ...
            if (m == null) {
                ...
                if (m == null) {
                    CivDynmap.log.info("Unable to create marker for:" + struct.getDisplayName() + " at " + struct.getCorner().toString());
                    continue;
                }
            }

            if (struct.getDisplayName() == null) {
                CivDynmap.log.info("display name was null");
                return;
            }
